...
const ContactForm = () => {
  ...
  const handleChangeInput = (e) => {
    ...
    setIsPaper(username.trim() || email.trim() || message.trim().length > 5);
    setError(!(username.trim() && email.trim() && message.trim().length > 5));
  };

  const handleSubmit = (e) => {
    e.preventDefault();

    if (!username.trim() || !email.trim() || message.trim().length <= 5) {
      setError(true);
      return;
    }
      ...
      <div className="contact_form">
        ...
        {!isFormSubmitted ? (
          <form onSubmit={handleSubmit} className="tn_form">
            ...
            {error && <p className="error-form">** {t("form_complete")} **</p>}
          </form>
        ) : (
          <motion.div
            animate={{ y: [65, 0] }}
